...
from .encode_attr_type import bin_to_seq, int_to_seq, float_to_seq, float_to_seq_sys
from .decode_attr_type import seq_to_bin, seq_to_number
class ParserPathD:
    ...
    def __float_to_seq(self, sign, coefficient, exponent):
        ...
        if length_bin & 1:
            return 'T' + bin_to_seq(ret)
        else:
            return 'C' + bin_to_seq(ret)
        
    def __seq_to_float(self, seq):
        length_nt = 1 if seq[0] == 'T' else 0
        seq = seq[1:]
        params = seq_to_bin(seq, 2)
        seq = seq[2:]
        sign = int(params[0])
        ...
        if self.type == 0:
            exponent = int(seq_to_bin(seq, 2), 2) - 6
            seq = seq[2:]
            value = str(int(seq_to_bin(seq, length_nt), 2))
            value = ('-' if sign == 1 else '') \
                    + value[0] + '.' + value[1:] \
                    + 'e' + str(exponent)
        else:
            value = seq_to_bin(seq, length_nt + exp_nt)
            exponent = self.offset_num
            if exp_bit > 0:
            ...
            value = ('-' if sign == 1 else '') \
                    + value \
                    + 'e' + str(exponent)
        return str(Decimal(value).normalize()), length_nt + exp_nt + 3    
    ...
    def encoder(self, string):
        ...
        ret = int_to_seq(False, len(data_list)) \
            + bin_to_seq(format(self.type * 2 + self.blank, '04b'))
        if self.type > 0 and self.type < 7:
            ret += bin_to_seq(format(self.offset_num + 6, '04b'))
            if self.blank == 1:
                ret += bin_to_seq(format(self.blank_num + 6, '04b'))
    ...
    def decoder(self, seq, start_idx = 0):
        ...
        seq = seq[idx:]
        type_bin = int(seq_to_bin(seq, 2), 2)
        seq = seq[2:]
        idx += 2
        self.type = type_bin >> 1
        self.blank = type_bin & 1
        if self.type > 0 and self.type < 7:
            self.offset_num = int(seq_to_bin(seq, 2), 2) - 6
            seq = seq[2:]
            idx += 2
            if self.blank == 1:
                self.blank_num = int(seq_to_bin(seq, 2), 2) - 6
                seq = seq[2:]
                idx += 2
    
        ret = ''
        for _ in range(0, length):
            tag = self.__decode_tag(seq[0:3])
            seq = seq[3:]
            ...
            for _ in range(0, params):
                if seq[0] == 'A' or seq[0] == 'G' or self.type == 7:
