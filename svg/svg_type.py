    ...
    def decode(self):
        ...
        if sub_seq[0] == 'A':
            if sub_seq[1] == 'C':
                ...
                ret = decoder.seq_to_float(sub_seq[1:1 + TYPECODE_LENGTH + FLOAT_LENGTH])
            else:
            ...
            tot = decoder.seq_to_int('A' + sub_seq[1:tot_int_length + TYPECODE_LENGTH + INT_SIZE_LENGTH])
            ret = []
            idx = tot_int_length + TYPECODE_LENGTH + INT_SIZE_LENGTH
            for _ in range(tot):
                if sub_seq[idx] == 'C':
                    ret.append(decoder.seq_to_float(sub_seq[idx:idx + TYPECODE_LENGTH + FLOAT_LENGTH]))
                    idx += TYPECODE_LENGTH + FLOAT_LENGTH
                else:
                    int_length = int(NT_BITS[sub_seq[idx + 1]] + NT_BITS[sub_seq[idx + 2]], 2)
                    ret.append(decoder.decode(sub_seq[idx:idx + int_length + TYPECODE_LENGTH + INT_SIZE_LENGTH]))
...
class SVGString(SVGType):
    ...
    def decode(self):
        ...
        str_length = 4*decoder.seq_to_int('A' + sub_seq[1:TYPECODE_LENGTH + INT_SIZE_LENGTH + size_int_length])
